...
├── AGENTS.md             # context for agents that land in this repo
├── CLAUDE.md -> AGENTS.md
└── README.md
```
...
`remote-workspace` 是一个让 Agent 真正“在远程服务器上干活”的 skill：先确认连接与
workspace，跑一次环境探测，然后按探测结果执行命令、改文件、管理长任务。

它解决的五个真实痛点：无法输入密码/2FA、瞎猜远程有没有 `rg`/`python`、破坏性地改文件、
把还在跑的后台任务说成“已完成”、以及停止任务时留下孤儿进程。

安全性：不代填密码、不改 `~/.ssh/config`；改文件一律备份 → 应用 → 读回 diff → 再删备份；
精确替换要求原文只出现一次，否则一个字都不写；长任务必须写出 `exit_code=0` 才算完成，
被 kill 的任务会报 `dead-without-exit-code` 而不是假装成功。

安装：把仓库 clone 到 `~/.claude/skills/remote-workspace`（或 `~/.pi/agent/skills/`、
`~/.codex/skills/`），然后用自然语言触发，例如
